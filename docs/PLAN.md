...
- **Subject-generic fallbacks (`crew/media_gapfill.py`):** the fallback layer
  itself is LLM-backed, so nothing subject-specific leaks in: a missing
  table/equation/chart spec is regenerated by **one extra call to the same
  LLM API** (validated, via the gatekeeper, logged like any call), and failed
  image queries get one LLM-suggested alternative query each before the
  configured image pool (the "bare-minimum images file") is used — and a
  chapter may simply end up without an image (the exercise needs ≥1 overall).
- **Dynamic topic (`--topic`) and the off-topic leak gate:** the CLI can
  override the subject per run. The research agent then also supplies the
  **bibliography** (`ResearchBrief.sources` → generated `references.bib`,
  citations filtered against it), the cover and running header use the
  outline title (`\BookTitle`), and `plan_is_topical=False` disables every
  subject-bound pool fallback — a Moon photo can never appear in a World War
  II book (verified end-to-end; the WW2 example in `examples/` is the proof).
