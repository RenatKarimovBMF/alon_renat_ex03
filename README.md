...
uv run python -m bookgen.main --live --topic "World War II"   # dynamic per-run topic (fully generic book)
```
...
**Generic subjects:** change `topic` — that's it for live runs. The crew
writes the prose and Hebrew summaries, specifies every image as a web search
query, supplies the table/equation/chart content, and chooses their placement;
anything missing is regenerated by **the same LLM API** (gap-fill), and failed
image searches get LLM-suggested alternative queries. The optional `extras`
block remains the bare-minimum image pool / offline-fixtures plan (Moon Race
by default), and a chapter can simply go without an image if everything fails.
...
- **Live-run examples** (each self-contained: `book.pdf`, chapters, session JSON, log, cost report):
  ...
  - [`examples/world_war_ii_causes_turning_points_and_legacy-20260613-003230/`](examples/world_war_ii_causes_turning_points_and_legacy-20260613-003230/) — **fully generic run via `--topic`** (no config edits): Claude Opus 4.8, 20 sheets / **16 subject pages**, $0.5690. Everything is topic-dynamic: agent-researched **bibliography** (8 real WW2 history books, all cited), web-fetched images (Versailles, Munich 1938), agent chart/table/equation, Hebrew summaries, dynamic cover + running header — and zero off-topic fallbacks (chapters whose image search failed simply have no image)
  - [`examples/the_yom_kippur_war_october_1973_and_its_aftermath-20260613-005121/`](examples/the_yom_kippur_war_october_1973_and_its_aftermath-20260613-005121/) — second `--topic` proof: Claude Opus 4.8, 20 sheets / **16 subject pages**, $0.8609. 8 real sources (Rabinovich, Herzog, Shazly, Kissinger, Sadat's and Dayan's memoirs…), 5 of 6 images from the web (incl. Sadat, period photos), agent table→ch3 / equation→ch4 / chart→ch5, topic-appropriate Hebrew summaries
